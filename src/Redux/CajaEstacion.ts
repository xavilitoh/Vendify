...
export interface CajaEstacion {
  ...
  descripcion: string;
  idSucursal: number;
  enable: boolean; // Se agregó opcional para poder editar estado
}
...
export const fetchCajaEstaciones = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "cajaEstaciones/fetchCajaEstaciones",
...
export const createCajaEstacion = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "cajaEstaciones/createCajaEstacion",
...
export const updateCajaEstacion = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "cajaEstaciones/updateCajaEstacion",
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateCajaEstacion.fulfilled, (state, action) => {
        const index = state.cajaEstaciones.findIndex(
          (caja) => caja.id === action.payload.id
        );
        if (index !== -1) {
...
export const selectCajaEstaciones = (state: RootState) =>
  state.cajasEstacion.cajaEstaciones;
